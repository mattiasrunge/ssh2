 ...
 *
 * Vendored so the chacha20-poly1305@openssh.com cipher needs no external
 * dependency: no runtime (including Node's node:crypto) exposes a standalone
 * Poly1305 primitive.
 *
 * Port of the Poly1305 implementation from @noble/ciphers 1.x
 * (https://github.com/paulmillr/noble-ciphers, MIT © Paul Miller), which is
 * based on the public-domain poly1305-donna-16
 * (https://github.com/floodyberry/poly1305-donna). Arithmetic uses 13-bit
 * limbs in 32-bit integer math so all intermediate products stay exact.
 */
...
class Poly1305 {
  ...
  update(data: Uint8Array): this {
    ...
    for (let pos = 0; pos < len;) {
      const take = Math.min(16 - this.pos, len - pos);
      // Fast path: at least one full block directly from input
      if (take === 16) {
        for (; 16 <= len - pos; pos += 16) this.process(data, pos);
        continue;
      }
...
/**
 * Compute the Poly1305 MAC of a message with a 32-byte one-time key.
 */
